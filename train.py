...
# vgg16_dbort_l1fullrow_ada.yaml
parser.add_argument('-c', '--config', default='/home/zbr/code/timm/vgg16_dborta_l1.yaml', type=str, metavar='FILE',
...
parser.add_argument('--data-dir', metavar='DIR', default='/home/zbr/datasets/imagenet_new/imagenet',
...
#model
group.add_argument('--model', default='vgg16', type=str, metavar='MODEL',
...
group.add_argument('--opt', default='BortA', type=str, metavar='OPTIMIZER',
...
# new by rqh
group.add_argument('--amptitude', type=float, default=-1.0,
                     help='amptitude (default: 1.0)')
# -1
group.add_argument('--mode', type=str, default='default',
                        help='mode for bort (default: default, col, row)')
group.add_argument('--gamma', type=float, default=1e-4,
                        help='gamma for bort (default: 1e-4)')

# for DBort
group.add_argument('--dbort', action='store_true', default=False,
                   help='Use DBort')
group.add_argument('--dbort-lambda', default=None, type=float, metavar='EPSILON',
                     help='DBort Epsilon (default: None, use opt default)')
group.add_argument('--dbort-beta', default=None, type=float, metavar='BETA',
...
group.add_argument('--lr', type=float, default=0.9, metavar='LR',
...
group.add_argument('--output', default='/home/zbr/code/result', type=str, metavar='PATH',
                   help='path to output folder (default: none, current dir)')
group.add_argument('--experiment', default='8.23exp', type=str, metavar='NAME',
...
def main():
    ...
    if args.distributed:
        ...
        _logger.info(
            ...
            f'Process {args.rank}, total {args.world_size}, device {device}.')
    else:
        args.rank = 0
    ...
    # # 检测是否有torch.cuda.is_available()，如果有则设置一些参数
    # # 如混合精度训练，使用cuda加速等
    # if torch.cuda.is_available():
    #     torch.backends.cuda.matmul.allow_tf32 = True
    #     torch.backends.cudnn.benchmark = True
...
def train_one_epoch(
    ...
    # print(f"has_no_sync: {has_no_sync}")
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        def _forward():
            with amp_autocast():
                ...
                if args.dbort:
                    loss = loss - args.dbort_lambda * (output.pow(2).mean()).pow(args.dbort_beta)
            if accum_steps > 1:
                loss /= accum_steps
            return loss
